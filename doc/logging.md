...
### Content Safety screening metric
...
`agent.contentsafety.evaluations` once per screening, tagged with a `stage` and an `outcome` dimension:
...
invert that and reject unscreened turns with the normal block message. Document screening has no fail-closed
...
**Content reaching the model unscreened** (Content Safety failed open — in `block` mode, blocking was off):

```kusto
customMetrics
| where name == "agent.contentsafety.evaluations"
| extend stage = tostring(customDimensions.stage), outcome = tostring(customDimensions.outcome)
| summarize screenings = sum(valueSum) by bin(timestamp, 15m), series = strcat(stage, "/", outcome)
| render timechart
```
...
```kusto
traces
| where message startswith "Prompt-injection screening flagged"
| project timestamp,
