...
export function PromptInput({
  ...
  accept?: string;
  /** Server-side `chatInput` cap, mirrored here so an over-long message is caught before the backend's 413. */
  maxLength?: number;
}) {
  ...
  // Count what actually goes on the wire (the trimmed text), so trailing whitespace never trips the cap.
  const length = value.trim().length;
  const overLimit = maxLength !== undefined && length > maxLength;
  // Only surface the counter as the cap gets close — it's noise the rest of the time.
  const showCounter = maxLength !== undefined && length > maxLength * 0.8;

  function submit(e?: FormEvent) {
    e?.preventDefault();
    const text = value.trim();
    if (!text || busy || disabled || overLimit) return;
    onSubmit(text);
    setValue('');
  }
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      {showCounter && (
        <div
          className={cn(
            'mb-1 pr-2 text-right text-xs tabular-nums',
            overLimit ? 'text-red-400' : 'text-[var(--color-muted)]',
          )}
        >
          {length.toLocaleString()} / {maxLength!.toLocaleString()}
          {overLimit && <span className="ml-1">— too long to send</span>}
        </div>
      )}
      <form
        onSubmit={submit}
        className="flex w-full items-end gap-2 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-2 shadow-lg"
      >
        {onAttachFiles && (
          <>
            <input
